...
from app.models.vision import (
    VisionBatchRequest, VisionBatchResponse, VisionResponse,
    WebDetection, WebEntity, WebImage, WebPage, BestGuessLabel, ErrorInfo
)
...
class VisionService:
    ...
    async def web_detection(self, request: VisionBatchRequest) -> VisionBatchResponse:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Vision API呼び出しエラー: {str(e)}"
            )
    
    def _convert_to_vision_requests(self, request: VisionBatchRequest) -> List[vision.AnnotateImageRequest]:
        ...
        for req in request.requests:
            ...
            # リクエスト作成
            vision_request = vision.AnnotateImageRequest(
                image=image,
                features=features
            )
        ...
        for image_response in response.responses:
            ...
            if image_response.error.message:
                vision_response.error = ErrorInfo(
                    code=image_response.error.code,
                    message=image_response.error.message
                )
            else:
                # Web Detection結果の変換
                if image_response.web_detection:
    ...
    def _convert_web_detection(self, web_detection) -> WebDetection:
        ...
        return WebDetection(
            ...
            bestGuessLabels=[
                BestGuessLabel(
                    label=label.label, 
                    languageCode=label.language_code
